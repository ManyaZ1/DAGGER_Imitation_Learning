...
class DaggerTrainer(MarioTrainer): # Κληρονομεί κυρίως για το test method!!!
    ...
    def __init__(self, config: DaggerConfig):
        ...
        self._behaviour_cloning_warmup(
            n_frames= 10_000,  #100_000,      # how many expert-labelled frames
            pretrain_steps=   500, #5_000   # how many offline gradient steps
            target_agreement=0.7 #0.70,  # stop when learner reaches this agreement with expert
        )
    ...
    def _behaviour_cloning_warmup(self,
        ...
        for step in range(1, pretrain_steps + 1):
            ...
            if step % check_interval == 0:
                acc = self._evaluate_learner_agreement(sample_size=256)
                print(f'  Step {step:04d} — agreement: {acc:.3f}')
                if acc >= target_agreement:
                    print(f'✅ Learner reached {acc*100:.1f}% agreement — stopping BC warm-up early.')
                    break

        self.learner.epsilon = 0.0  # lock in greedy policy

    def _evaluate_learner_agreement(self, sample_size: int = 256) -> float:
        import random
        batch = random.sample(self.learner.dagger_memory, min(sample_size, len(self.learner.dagger_memory)))
        states, labels = zip(*batch)

        states_tensor = torch.FloatTensor(np.array(states)).to(self.learner.device)
        labels_tensor = torch.LongTensor(labels).to(self.learner.device)

        with torch.no_grad():
            logits = self.learner.q_network(states_tensor)
            preds = logits.argmax(dim=1)

        matches = (preds == labels_tensor).sum().item()
        return matches / len(labels)

    def _behaviour_cloning_warmup_hardcoded(self,
                              n_frames: int = 100_000,
                              pretrain_steps: int = 5_000):
        print(f'Collecting {n_frames} expert frames for BC …')
        frames_collected = 0
    ...
    def _setup_metrics(self):
        ...
        return
    # dagger_trainer.py  (inside class DaggerTrainer)
    def test(self,
            n_episodes: int = 5,
            render: bool = False,
            record: bool = False):
        ...
        for ep in range(n_episodes):
            ...
            while not done:
                # --- learner sees partial obs only ---
                state_part = (self.observation_wrapper.transform_observation(state_full)
                            if self.observation_wrapper else state_full)

                #action = self.agent.act(state_part, training=False)   # deterministic
                action = self.learner.act(state_part, training=False)
                # state_full, reward, done, _, _ = self.env.step(action)
                state_full, reward, done, info = self.env.step(action)
