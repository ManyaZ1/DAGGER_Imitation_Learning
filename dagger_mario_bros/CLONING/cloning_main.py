...
class MarioBehaviorCloningExperiment:
    ...
    def step2_train_behavior_cloning_agents(self, 
        ...
        # 
        self.plot_all_agents_validation_accuracy(bc_agents)
    ...
    def _plot_evaluation_results(self, results):
        ...
        plt.savefig(f'plots_bc/bc_evaluation_results_{timestamp}.png', dpi=300, bbox_inches='tight')
        #plt.show()
    ...
    def plot_all_agents_validation_accuracy(self,training_results_dict):
        ...
        for name, agent in training_results_dict.items():
            ...
            plt.plot(epochs, agent.validation_accuracies, label=name)

        plt.xlabel('Epoch')
        plt.ylabel('Validation Accuracy')
        plt.title('Validation Accuracy Comparison Across Agents')
        plt.legend()
        plt.grid(True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        plt.savefig(f'plots_bc/combined_validation_accuracy_{timestamp}.png', dpi=300, bbox_inches='tight')
        plt.show()
